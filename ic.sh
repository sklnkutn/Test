...
rotate_logs_if_needed() {
    ...
    max_size_bytes=$((LOG_MAX_SIZE_KB * 1024))
    (( max_size_bytes > 0 )) || return 0

    local current_size
    current_size=$(wc -c <"$LOG_FILE" 2>/dev/null || echo 0)
    is_non_negative_int "$current_size" || current_size=0

    (( current_size < max_size_bytes )) && return 0

    local i
    for (( i = LOG_KEEP_FILES; i >= 1; i-- )); do
...
load_config() {
    ...
    while IFS= read -r raw_line || [[ -n "$raw_line" ]]; do
        ...
        case "$line" in
            ...
            LIMIT_RESULTS=*) LIMIT_RESULTS="${line#LIMIT_RESULTS=}" ;;
            MAX_RESULTS=*) LIMIT_RESULTS="${line#MAX_RESULTS=}" ;;
            ...
            LOG_MAX_SIZE_KB=*) LOG_MAX_SIZE_KB="${line#LOG_MAX_SIZE_KB=}" ;;
            LOG_KEEP_FILES=*) LOG_KEEP_FILES="${line#LOG_KEEP_FILES=}" ;;
    ...
    if ! is_non_negative_int "$DEDUP_CYCLES"; then
        DEDUP_CYCLES="$DEFAULT_DEDUP_CYCLES"
        log_message "DEDUP_CYCLES некорректен. Использую default $DEFAULT_DEDUP_CYCLES."
    fi

    if ! is_non_negative_int "$LOG_MAX_SIZE_KB" || (( LOG_MAX_SIZE_KB <= 0 )); then
        LOG_MAX_SIZE_KB="$DEFAULT_LOG_MAX_SIZE_KB"
        log_message "LOG_MAX_SIZE_KB некорректен. Использую default $DEFAULT_LOG_MAX_SIZE_KB KB."
    fi

    if ! is_non_negative_int "$LOG_KEEP_FILES" || (( LOG_KEEP_FILES <= 0 )); then
        LOG_KEEP_FILES="$DEFAULT_LOG_KEEP_FILES"
        log_message "LOG_KEEP_FILES некорректен. Использую default $DEFAULT_LOG_KEEP_FILES."
    fi
...
send_telegram() {
    ...
    if [[ -z "$BOT_TOKEN" || -z "$CHAT_ID" ]]; then
        log_message "BOT_TOKEN/CHAT_ID не заданы. Пропускаю отправку в Telegram."
        return 1
    fi
    ...
    response="$(curl -sS -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" \
        -d chat_id="${CHAT_ID}" \
        -d text="$message" \
        -d parse_mode="${TG_PARSE_MODE}" 2>/dev/null || true)"

    if [[ -n "$response" ]] && jq -e . >/dev/null 2>&1 <<<"$response"; then
        ok="$(jq -r '.ok // false' <<<"$response")"
        description="$(jq -r '.description // empty' <<<"$response")"
    fi
    ...
    # Частая проблема: parse_mode=Markdown ломается на спецсимволах в тексте.
    response="$(curl -sS -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" \
        -d chat_id="${CHAT_ID}" \
    ...
    if [[ -n "$response" ]] && jq -e . >/dev/null 2>&1 <<<"$response"; then
        ...
        ok="false"
        description="Пустой или невалидный ответ Telegram API"
    fi

    if [[ "$ok" == "true" ]]; then
        log_message "Telegram: отправка успешна после повтора без parse_mode."
        return 0
    fi

    log_message "Ошибка отправки в Telegram: ${description:-неизвестная ошибка}"
    return 1
}
...
while true; do
    ...
    log_message "Команда: vastai search offers \"$SEARCH_QUERY\" --order price+ --raw --type $SEARCH_TYPE --limit $LIMIT_RESULTS"
    OFFERS="$(vastai search offers "$SEARCH_QUERY" --order price+ --raw --type "$SEARCH_TYPE" --limit "$LIMIT_RESULTS" 2>/dev/null || true)"
    ...
    if (( OFFER_COUNT >= 1 )); then
        ...
        if (( CYCLES_LEFT > 0 )); then
            ...
            if send_telegram "$MSG"; then
                ...
                log_message "Уведомление не отправлено, dedup не обновлён — повторим в следующем цикле."
            fi
        fi
    else
